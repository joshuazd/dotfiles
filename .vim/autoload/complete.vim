...
function! complete#fzf_window() abort
  let args = {'border':[],
        ...
        \'pos': 'topleft',
        \'line': 'cursor+1',
        \'col': 'cursor-'.(col('.') <= g:complete_prefix_length + 2 ? col('.') - 1 : g:complete_prefix_length + 2),
  ...
  unlet args.border
  let args.col = 'cursor'.(col('.') <= g:complete_prefix_length + 2 ? (col('.') <= 3 ? '+'.(3-col('.')) : '-'.(col('.')-3)) : '-'.(g:complete_prefix_length))
  let args.minwidth -= 2
  " let args.minheight -= 2
  let args.line = 'cursor+2'
