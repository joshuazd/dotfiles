...
" Language:	XML
" Maintainer:	Joshua Zink-Duda
" Last Change:	2018 Nov 8
" Version: 1.0
"
" Changelog:
" 1.9 - 2007 Aug 15
...
let mediators = ['property', 'call-template', 'filter', 'payloadFactory', 'respond', 'store', 'send', 'call', 'log', 'sequence', 'enrich', 'xslt', 'script', 'class', 'header', 'dbreport', 'dblookup', 'datamapper']
...
function! xml#CompleteTags(findstart, base)
  " echom a:base
  if a:findstart
    ...
    if context =~? '\s'
      ...
      if attr =~? "=\s*[\"']" || attr =~? '=\s*$'
        ...
        if tag =~? '^[?!]'
          ...
          if has_key(b:xml_omni, tag) && has_key(b:xml_omni[tag][1], attrname)
            let values = b:xml_omni[tag][1][attrname]
          else
            return []
          endif
        endif
      ...
      elseif tag =~? '^!'
        ...
        if !has_key(b:xml_omni, tag)
          " Abandon when data file isn't complete
          return []
        endif
        let attrs = keys(b:xml_omni[tag][1])
      endif
      ...
      let menu = res + res2
      let final_menu = []
      if has_key(b:xml_omni, 'vimxmlattrinfo')
        for i in range(len(menu))
          let item = menu[i]
          if has_key(b:xml_omni['vimxmlattrinfo'], item)
            let m_menu = b:xml_omni['vimxmlattrinfo'][item][0]
            let m_info = b:xml_omni['vimxmlattrinfo'][item][1]
          else
            let m_menu = ''
            let m_info = ''
          endif
          if tag !~? '^[?!]' && len(b:xml_omni[tag][1][item]) > 0 && b:xml_omni[tag][1][item][0] =~ '^\(BOOL\|'.item.'\)$'
            let item = item
          else
            let item .= '="'
          endif
    ...
    if opentag ==? ''
      "return []
      let tags = keys(b:xml_omni)
      call filter(tags, "v:val !~? '^vimxml'")
    else
      if !has_key(b:xml_omni, opentag)
        " Abandon when data file isn't complete
        return []
      endif
      let tags = b:xml_omni[opentag][0]
    endif
    ...
    if has_key(b:xml_omni, 'vimxmltaginfo')
      echom 'here'
      let final_menu = []
      for i in range(len(menu))
        let item = menu[i]
        echom item
        let end = ''
        if has_key(b:xml_omni, item) && len(b:xml_omni[item][1]) == 0
          let end = '>'
        endif
        if has_key(b:xml_omni['vimxmltaginfo'], item)
          let m_menu = b:xml_omni['vimxmltaginfo'][item][0]
          let m_info = b:xml_omni['vimxmltaginfo'][item][1]
          if has_key(b:xml_omni, item) && len(b:xml_omni[item][1]) == 0
            let end = b:xml_omni['vimxmltaginfo'][item][0]
          endif
        else
          let m_menu = ''
          let m_info = ''
        endif
        let final_menu += [{'word':xml_namespace.item.end, 'menu':m_menu, 'info':m_info}]
      endfor
    else
...
function! xml#GetLastOpenTag(unaryTagsStack)
  let linenum=line('.')
