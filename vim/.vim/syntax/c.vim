...
" nextgroup=cCondParam
syn keyword	cRepeat		while for do 
" nextgroup=cFuncParams

syn keyword	cTodo		contained TODO FIXME XXX

syn match       cPunct ";\|\.\|,"

" syn region      cCondParam contained matchgroup=cPunct start="\s*(" end=")" contains=cType,cPunct,cOperator,cString,cConstant,cNumber,cFunc,cCharacter

syn match       cFunc "\h\w*(\@=" contains=@cFuncName "nextgroup=cFuncParams
syn match       cFunc "\h\w*\ze(" contains=@cFuncName nextgroup=cParen
" syn region      cFuncParams contained matchgroup=cPunct start="(" end=")" contains=cParamList
" syn match       cParamList contained "[^)]*" contains=cFuncParams,cType,cPunct,cOperator,cString,cCharacter,cConstant,cNumber,cIdentifier,cDereference,cStructure skipwhite
...
if exists('c_curly_error')
  ...
  syn region	cBlock		matchgroup=cPunct start="{" end="}" contains=ALLBUT,cBadBlock,cCurlyError,@cParenGroup,cErrInParen,cCppParen,cErrInBracket,cCppBracket,@cStringGroup,@Spell fold
else
...
elseif exists('c_no_bracket_error')
  ...
  if s:ft ==# 'cpp' && !exists('cpp_no_cpp11')
    syn region	cParen		matchgroup=cPunct transparent start='(' end=')' contains=ALLBUT,cParamList,cFuncParams,@cParenGroup,cCppParen,cErrInBracket,cCppBracket,@cStringGroup,@Spell
    " cCppParen: same as cParen but ends at end-of-line; used in cDefine
    syn region	cCppParen	matchgroup=cPunct transparent start='(' skip='\\$' excludenl end=')' end='$' contained contains=ALLBUT,@cParenGroup,cErrInBracket,cParen,cBracket,cString,@Spell
    syn match	cParenError	display "[\])]"
    syn match	cErrInParen	display contained "<%\|%>"
    ...
    syn region	cParen		matchgroup=cPunct transparent start='(' end=')' end='}'me=s-1 contains=ALLBUT,cParamList,cFuncParams,cBlock,@cParenGroup,cCppParen,cErrInBracket,cCppBracket,@cStringGroup,@Spell
    " cCppParen: same as cParen but ends at end-of-line; used in cDefine
    syn region	cCppParen	matchgroup=cPunct transparent start='(' skip='\\$' excludenl end=')' end='$' contained contains=ALLBUT,cParamList,@cParenGroup,cErrInBracket,cParen,cBracket,cString,@Spell
    syn match	cParenError	display "[\])]"
    syn match	cErrInParen	display contained "[\]{}]\|<%\|%>"
